...
class HandLandmarkerHelper(
    ...
    private var lastDetectedText: Text? = null
    private var tts: TextToSpeech? = null
    private var lastExecutionTime = 0L
    private val delayInMillis = 4000L
    private var lastExecutionTime2 = 0L
    private val delayInMillis2 = 1000L
    init {
        setupHandLandmarker()
    ...
    fun detectLiveStream(
        ...
        if ((currentTime2 - lastExecutionTime2) >= delayInMillis2) {
            lastExecutionTime2 = currentTime2
            //detext text
            val recognizer_text = TextRecognition.getClient(TextRecognizerOptions.DEFAULT_OPTIONS)
            val image = InputImage.fromBitmap(bitmapBuffer, 0)
            val result = recognizer_text.process(image)
                .addOnSuccessListener { visionText ->
                    // Task completed successfully
                    // ...
                    lastDetectedText = visionText

                }.addOnFailureListener { e ->
                    // Task failed with an exception
                    // ...
                    Log.d("error apple", e.toString())
                }
        }
    ...
    private fun returnLivestreamResult(
        ...
        try{
            val resultText = lastDetectedText?.text
            if (resultText != null) {
                var minDistance = 10000000.0
                //lastTime = timenow
                var closestBlock = lastDetectedText?.textBlocks?.get(0)
                    for (block in lastDetectedText?.textBlocks!!) {
                        val blockText = block.text
                        //Log.d("banana", block.boundingBox!!.centerY().toString())
                        val dis = distance(
                            block.boundingBox!!.centerX(),
                            block.boundingBox!!.centerY(),
                            (result.landmarks()[0][8].x() * 640).toInt(),
                            (result.landmarks()[0][8].y() * 420).toInt()
                        )
                        if (dis < minDistance) {
                            minDistance = dis
                            closestBlock = block
...
//                        val blockCornerPoints = closestBlock?.cornerPoints
//                        val blockFrame = closestBlock?.boundingBox
                if (closestBlock != null) {
                    Log.d("See that", closestBlock.text)
                    tts!!.speak(closestBlock.text, TextToSpeech.QUEUE_FLUSH, null,"")
//                    for (line in closestBlock.lines) {
//                        val lineText = line.text
//                        Log.d("See this", lineText)
